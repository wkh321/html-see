...
- Category: Operations & Deployment | Build Methods | Troubleshooting & Debugging
- Instructions:
  - 打开绘制器为「点鸭一次性令牌」方案（URL 不嵌套 u/p）：门户 issueOpenToken（projects.js/app.js）在点鸭 logs 表启用时先清理该用户全部旧 open_ticket 再 insert（type='open_ticket'、user_id=<uid>、name=<token>、status='active'、detail=JSON{folder,exp,ip}，15min TTL，ip 用全局 getClientIP 超时 8s 回退空串），URL 组装为 `plotter/index.html?t=<token>&id=<folder>`；点鸭未启用或 insert 失败直接 reject 并 toast（无回退票据，u/p 永不进 URL）。
  - 绘制器侧 `plotter/dbticket.js`：`PlotterTicket.verify(token, projectId, getIp)` 三参，校验 active+未过期+detail.folder===projectId+IP 匹配（无绑定 IP 首次校验写入绑定；获取不到当前 IP 且已绑定 → 拒绝），成功置 consumed 并 remove 删除记录；全程 `Promise.resolve().then()` 包裹防 DbsApi.init 同步 throw 冒泡；10s withTimeout 防加载卡死返回 {timeout:true}。`__dbsConfigs['logs']` 需从 `window.module.exports` 回填，否则 init({table:'logs'}) 错用 users 配置。
  - script.js 外部模式 IIFE：只接受 ?t=&id=（new=1 或无参数进空白新项目，只带 t 或只带 id 或旧 u/p 链接一律全屏锁屏），删除 sessionStorage 刷新放行——刷新/重开必须重新签发令牌；30s 看门狗（extArmWatchdog/extDisarmWatchdog）兜底锁屏；extOpen 缺配置补 extHideLoading；verify 链补 catch。顶部导航（topnav.js/app.js）返回/品牌/绘图主页按钮改跳 `plotter/index.html?new=1`。
  - plotter/index.html 脚本顺序：dbs-all.js → dbs-users.js → auth.js → dbticket.js → script.js → mdeditor.js。app.js 为构建合并产物，改 projects.js/topnav.js 源码后须同步 app.js 对应段。
  - 测试脚本：`/tmp/opencode/dbticket.test.js`（14 场景：三参/IP 绑定/项目不符/同步 throw）、`/tmp/opencode/ext-mode.test.js`（jsdom 13 场景，jsdom 在 /tmp/opencode/dbs-test/node_modules，看门狗用 speedUpWatchdog 压缩 30s）、`/tmp/opencode/issue-token.test.mjs`（vm 提取 app.js 函数体测 6 场景，依赖真实 dbstable，场景间需 mod.resetDb() 清 dbCache）。
