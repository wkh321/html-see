...
(function (global) {
  ...
  async function ghDelete(cfg, path, message) {
    ...
    var res = await fetch(apiUrl(cfg, path), { method: 'DELETE', headers: ghHeaders(cfg), body: JSON.stringify(body) });
  ...
  // 列出当前用户的云端项目：{name, id, savedAt, path}
  async function listCloudProjects() {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var raw = await ghRead(cfg, buildPath(dir, f.name));
      var obj = null;
      ...
      list.push({
        ...
        path: buildPath(dir, f.name)
      });
    }
    list.sort(function (a, b) { return (b.savedAt || 0) - (a.savedAt || 0); });
    return list;
  }
  // 按唯一 ID 读取云端项目（不按名称检索）
  async function readCloudProject(id) {
    var cfg = requireConfig();
    var list = await listCloudProjects();
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === String(id)) {
        var raw = await ghRead(cfg, list[i].path);
        var obj = null;
        try { obj = JSON.parse(raw); } catch (e) {}
        if (!obj) throw new Error('云端项目读取失败');
        return { project: obj, name: list[i].name, id: list[i].id, savedAt: list[i].savedAt };
      }
    }
    throw new Error('未找到 ID 为 ' + id + ' 的云端项目');
  }
  // 保存云端项目：重名检测（排除自身 _id）；无 _id 时自动生成唯一 ID
  async function writeCloudProject(name, data) {
    var cfg = requireConfig();
    var s = getSession();
    if (!s || !s.userId) throw new Error('未登录');
    var n = sanitizeName(name);
    ...
    await ghWrite(cfg, projectFilePath(cfg, s.userId, n), JSON.stringify(data, null, 2), 'Save project ' + n);
    return data;
  }
  ...
  function loadAdminConfig() {
    var cfg = getConfig();
    if (!cfg) return;
    ['admToken', 'admOwner', 'admRepo', 'admBranch', 'admUsersRoot', 'admAvatarsRoot', 'admShareRoot'].forEach(function (id) {
      var key = { admToken: 'token', admOwner: 'owner', admRepo: 'repo', admBranch: 'branch', admUsersRoot: 'usersRoot', admAvatarsRoot: 'avatarsRoot', admShareRoot: 'shareRoot' }[id];
      var el = document.getElementById(id);
      if (el) el.value = cfg[key] || (id === 'admShareRoot' ? 'share' : '');
    });
